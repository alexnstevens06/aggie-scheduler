...
//     // TLS verification on (good practice)
...
//         // The endpoint returns JSON. Dump once if you want to inspect:
//         // std::cout << response << "\n";

//         json j = json::parse(response);

//         // The exact keys depend on the server’s JSON.
//         // Two common patterns are:
//         // 1) j is an array of rows
//         // 2) j is an object containing an array (e.g., j["rows"])
//         auto as_array_ptr = j.is_array() ? &j : (j.contains("rows") && j["rows"].is_array() ? &j["rows"] : nullptr);
//         if (!as_array_ptr) {
//             std::cerr << "Unexpected JSON shape. Here is the first 2KB to inspect:\n"
//                       << response.substr(0, 2000) << "\n";
//             curl_global_cleanup();
//             return "Unexpected JSON shape. Here is the first 2KB to inspect:\n";
//         }

//         const json& rows = *as_array_ptr;

//         // Print CSV header (adjust to the actual keys you see)
//         std::cout << "Year,Semester,Prof,GPA,A,B,C,D,F,I,Q,S,U,X,Section\n";

//         for (const auto& r : rows) {
//             // Use .value(key, default) so it won't crash if a key is missing
//             int year           = r.value("year", 0);
//             std::string sem    = r.value("semester", "");
//             std::string prof   = r.value("prof", r.value("instructor", ""));
//             double gpa         = r.value("gpa", 0.0);
//             int A = r.value("A", 0), B = r.value("B", 0), C = r.value("C", 0),
//                 D = r.value("D", 0), F = r.value("F", 0), I = r.value("I", 0),
//                 Q = r.value("Q", 0), S = r.value("S", 0), U = r.value("U", 0),
//                 X = r.value("X", 0);
//             int section        = r.value("section", r.value("Section", 0));
